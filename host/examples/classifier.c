...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    float avg_topk = 0;
    int *indexes = (int*)calloc(topk, sizeof(int));
    double btime=what_time_is_it_now();
    //printf("begin time is %lf\n",btime);
    for(i = 0; i < m; ++i){
        int class = -1;
        char *path = paths[i];
        for(j = 0; j < classes; ++j){
        ...
        printf("%d: top 1: %f, top %d: %f\n", i, avg_acc/(i+1), topk, avg_topk/(i+1));
        //printf("Pridict time %lf\n",what_time_is_it_now()-btime);
    }
#ifdef SECURITY
    if(net->thread_pool) destroy_tpool(net->thread_pool);
    if(net->ctx_util) close_TEE_ctx_util(net->ctx_util);
#endif
    printf("Vaildate %d pictures in %lf seconds\n",m,what_time_is_it_now()-btime);
}
...
void label_classifier(char *datacfg, char *filename, char *weightfile)
{
    ...
    if(net->ctx_util) close_TEE_ctx_util(net->ctx_util);
#endif
    printf("Predicted %d pictures in %lf\n",m,what_time_is_it_now()-btime);
}
