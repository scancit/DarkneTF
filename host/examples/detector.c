...
static void print_cocos(FILE *fp, char *image_path, detection *dets, int num_boxes, int classes, int w, int h)
{
    ...
    for(i = 0; i < num_boxes; ++i){
        ...
        for(j = 0; j < classes; ++j){
            ...
            //if (dets[i].prob[j]) fprintf(fp, "{\"image_path\": \"%s\", \"category_id\":%d, \"bbox\":[%f, %f, %f, %f], \"score\":%f},\n", image_path, coco_ids[j], bx, by, bw, bh, dets[i].prob[j]);
        }
    }
}
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    //     else{
    //         iswait=0;
    //         while(!is_taskover(net->thread_pool)){
    //             usleep(20000);
    //         }
    //     }
    // #endif
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        float *X = sized.data;
        nowTime=what_time_is_it_now();
        if (l.type == DETECTION || l.type == YOLO) {
            network_predict(net, X);
        }
        if (l.type == YOLO4) {
            network_predict_y4(net, X);
        }
        printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-nowTime);
...
void label_detector(char *datacfg, char *cfgfile,char *weightfile,float thresh,float hier_thresh){
    list *options=read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char *test_images=option_find_str(options, "test","data/test.list");
    char **names = get_labels(name_list);
    ...
    free_list(plist);
    int iswait=0;
    double nowtime=what_time_is_it_now();
    for(int index=0;index<list_size;index++){
        ...
        print_cocos(stderr,input,dets,nboxes,l.classes,im.w,im.h);
        free_detections(dets,nboxes);
        free_image(im);
        if (resize) free_image(sized);
    #ifdef SECURITY
    ...
    destroy_tpool(net->thread_pool);
#endif
    printf("Detect %d pictures in %lf seconds\n",list_size,what_time_is_it_now()-nowtime);
#ifdef SECURITY
    close_TEE_ctx_util(net->ctx_util);
#endif
}
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "label")) label_detector(datacfg,cfg,weights,thresh,hier_thresh);
