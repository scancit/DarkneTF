...
void forward_network(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
		...
		const char* layerName[] = { "CONVOLUTIONAL","MAXPOOL","AVGPOOL","SOFTMAX", "DETECTION", "ROUTE", "COST", "NORMALIZATION", "LOCAL", "SHORTCUT", "ACTIVE", "BATCHNORM", "NETWORK", "YOLO", "YOLO4", "UPSAMPLE", "LOGXENT", "BLANK"};
...
{
    layer l = net.layers[i];
    if(l.type != YOLO && l.type != YOLO4) opencl_pull_array(l.output_gpu, l.output, l.outputs*l.batch);
    return l.output;
}
...
detection *make_network_boxes_y4(network *net, float thresh, int *num)
{
    ...
    for (i = 0; i < net->n; ++i) {
        ...
        if (l_tmp.type == YOLO || l_tmp.type == YOLO4 || l_tmp.type == DETECTION) {
            l = l_tmp;
            break;
        }
    }
