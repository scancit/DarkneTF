...
dbox derivative_y4(box a, box b)
{
    dbox d;
    d.dx = 0;
    d.dw = 0;
...
float overlap_y4(float x1, float w1, float x2, float w2)
{
    float l1 = x1 - w1/2;
    float l2 = x2 - w2/2;
    float left = l1 > l2 ? l1 : l2;
...
float box_intersection_y4(box a, box b)
{
    float w = overlap_y4(a.x, a.w, b.x, b.w);
    float h = overlap_y4(a.y, a.h, b.y, b.h);
    if(w < 0 || h < 0) return 0;
    float area = w*h;
    return area;
}

float box_union_y4(box a, box b)
{
    float i = box_intersection_y4(a, b);
    float u = a.w*a.h + b.w*b.h - i;
    return u;
}
...
float box_iou_kind_y4(box a, box b, IOU_LOSS iou_kind)
{
    //IOU, GIOU, MSE, DIOU, CIOU
    switch(iou_kind) {
        case IOU: return box_iou_y4(a, b);
        case GIOU: return box_giou_y4(a, b);
        case DIOU: return box_diou_y4(a, b);
        case CIOU: return box_ciou_y4(a, b);
...
{
    //return box_intersection_y4(a, b)/box_union(a, b);

    float I = box_intersection_y4(a, b);
    float U = box_union_y4(a, b);
    if (I == 0 || U == 0) {
        return 0;
    }
    return I / U;
...
dxrep dx_box_iou_y4(box pred, box truth, IOU_LOSS iou_loss) {
    boxabs pred_tblr = to_tblr(pred);
    float pred_t = fmin(pred_tblr.top, pred_tblr.bot);
    float pred_b = fmax(pred_tblr.top, pred_tblr.bot);
    float pred_l = fmin(pred_tblr.left, pred_tblr.right);
    ...
    boxabs truth_tblr = to_tblr(truth);
#ifdef DEBUG_PRINTS
    printf("\niou: %f, giou: %f\n", box_iou(pred, truth), box_giou_y4(pred, truth));
    printf("pred: x,y,w,h: (%f, %f, %f, %f) -> t,b,l,r: (%f, %f, %f, %f)\n", pred.x, pred.y, pred.w, pred.h, pred_tblr.top, pred_tblr.bot, pred_tblr.left, pred_tblr.right);
    printf("truth: x,y,w,h: (%f, %f, %f, %f) -> t,b,l,r: (%f, %f, %f, %f)\n", truth.x, truth.y, truth.w, truth.h, truth_tblr.top, truth_tblr.bot, truth_tblr.left, truth_tblr.right);
#endif
    //printf("pred (t,b,l,r): (%f, %f, %f, %f)\n", pred_t, pred_b, pred_l, pred_r);
...
//#endif
    ...
    ddx.dt = p_dx;      //We follow the original code released from GDarknet. So in yolo4_layer.c, dt, db, dl, dr are already dx, dy, dw, dh.
    ddx.db = p_dy;
    ddx.dl = p_dw;
    ddx.dr = p_dh;
...
dbox dintersect_y4(box a, box b)
{
    float w = overlap(a.x, a.w, b.x, b.w);
    float h = overlap(a.y, a.h, b.y, b.h);
    dbox dover = derivative(a, b);
...
int nms_comparator_y4(const void *pa, const void *pb)
{
...
void do_nms_obj_y4(detection *dets, int total, int classes, float thresh)
{
    int i, j, k;
    k = total - 1;
    for (i = 0; i <= k; ++i) {
...
void do_nms_sort_y4(detection *dets, int total, int classes, float thresh)
{
    int i, j, k;
    k = total - 1;
    for (i = 0; i <= k; ++i) {
        ...
        for (i = 0; i < total; ++i)
        {
            ...
            for (j = i + 1; j < total; ++j) {
                ...
                if (box_iou(a, b) > thresh && nms_kind == CORNERS_NMS)
                {
                    ...
                    /*
                    if (dets[j].points == YOLO4_CENTER && (dets[i].points & dets[j].points) == 0) {
                        dets[i].bbox.x = (dets[i].bbox.x*alpha_prob + dets[j].bbox.x*beta_prob);
                        dets[i].bbox.y = (dets[i].bbox.y*alpha_prob + dets[j].bbox.y*beta_prob);
                    }
            ...
            //if ((nms_kind == CORNERS_NMS) && (dets[i].points != (YOLO4_CENTER | YOLO4_LEFT_TOP | YOLO4_RIGHT_BOTTOM)))
            //    dets[i].prob[k] = 0;
        }
    }
}
