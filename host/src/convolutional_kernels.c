...
void forward_convolutional_layer_gpu(convolutional_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        for(j = 0; j < l.groups; ++j){
            ...
            if (l.size == 1){
            ...
            #ifdef SECURITY
                float *fa=(float*)clEnqueueMapBuffer(a.que,a.mem,CL_TRUE,CL_MAP_READ,0,sizeof(float)*m*k,0,NULL,NULL,&ert);
                float* fb=(float*)clEnqueueMapBuffer(b.que,b.mem,CL_TRUE,CL_MAP_READ,0,sizeof(float)*n*k,0,NULL,NULL,&ert);
                pre_sec_copy(net.ctx_util,net.thread_pool,fa,fb,m,k,n,l.which_layer);
                clEnqueueUnmapMemObject(b.que,b.mem,fb,0,NULL,NULL);
                clEnqueueUnmapMemObject(a.que,a.mem,fa,0,NULL,NULL);
            #endif
                ...
                gemm_offset_gpu(0,0,m,n,k,
                                1,
                                a,j*l.nweights/l.groups,k,
                                b,0,n,
                                1,
                                c,(i*l.groups + j)*n*m,n);
            }
        #ifdef ATTACK
            attack_simulation(a.que,a.mem,b.mem,c.mem,m,n,k,l.which_layer);
        #endif

        #ifdef SECURITY
            float *fc=(float*)clEnqueueMapBuffer(c.que,c.mem,CL_TRUE,CL_MAP_READ,0,sizeof(float)*n*m,0,NULL,NULL,&ert);
            // for(int index=0;index<m*n;index+=1000){
            //     printf("%f ",fc[index]);
            // }
            // printf("\n");
            security_fun(net.ctx_util,net.thread_pool,fc,m,k,n,l.which_layer,net.weights_file,net.image_name);
            clEnqueueUnmapMemObject(c.que,c.mem,fc,0,NULL,NULL);
        #endif
        }
    }
