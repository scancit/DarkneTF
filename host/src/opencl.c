...
cl_mem_ext opencl_make_array(float *x, size_t n)
{
    ...
    buf.org = clCreateBuffer(opencl_context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,buf.len * buf.obs, NULL,&clErr);
    if (clErr != CL_SUCCESS)
        printf("couldn't create buffer on device. error: %s\n", clCheckError(clErr));

    clErr = clEnqueueWriteBuffer(*opencl_queues,buf.org, CL_TRUE,0, buf.obs*buf.len, buf.ptr, 0, NULL, NULL);
    if (clErr != CL_SUCCESS)
        printf("couldn't wirite buffer on device. error: %s\n", clCheckError(clErr));
...
cl_mem_ext opencl_make_int_array(int *x, size_t n)
{
    ...
    buf.org = clCreateBuffer(opencl_context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
                             buf.len * buf.obs, NULL,
                             &clErr);
